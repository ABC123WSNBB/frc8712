...
const module=mountPlanGraph($('#host'),{plans,onSelect(plan){const detail=$('#host-detail');detail.replaceChildren();const h=document.createElement('h2');h.textContent=plan.title;const p=document.createElement('p');p.textContent=`${plan.date}\n${plan.notes||'把目标分成一个可以完成的小步骤。'}`;detail.append(h,p);},onStatus:status=>$('#status').textContent=status,onGesture:state=>{$('#status').textContent=({lost:'● 未检测到手 · 已暂停',point:'● 指向计划 · 停留选择',pinchRotate:'● 捏住转腕 · 旋转',zoomIn:'● 张开 · 放大',zoomOut:'● 收拢 · 缩小',rotate:'● 转腕 · 旋转'})[state.mode]||'● 手势已就绪';}});
